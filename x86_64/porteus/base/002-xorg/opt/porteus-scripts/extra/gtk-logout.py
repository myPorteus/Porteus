...
class LogoutDialog(Gtk.Window):
    ...
    def on_Shutdown_clicked(self, button):
        subprocess.run(['dbus-send', '--system', '--print-reply', '--dest=org.freedesktop.ConsoleKit', '/org/freedesktop/ConsoleKit/Manager', 'org.freedesktop.ConsoleKit.Manager.Stop'])
        
    def on_Reboot_clicked(self, button):
        subprocess.run(['dbus-send', '--system', '--print-reply', '--dest=org.freedesktop.ConsoleKit', '/org/freedesktop/ConsoleKit/Manager', 'org.freedesktop.ConsoleKit.Manager.Restart'])

    def on_Suspend_clicked(self, button):
        subprocess.run(['dbus-send', '--system', '--print-reply', '--dest=org.freedesktop.UPower', '/org/freedesktop/ConsoleKit/UPower', 'org.freedesktop.UPower.Suspend'])
